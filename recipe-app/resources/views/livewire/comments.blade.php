    ...
    <div class="space-y-4">
        @if (!$recipe->comments()->exists())
            <div
                ...
                <i class="fas fa-info-circle mr-1"></i> {{ __('main.no_comment') }}
            </div>
        @endif

        <form wire:submit.prevent="addComment">
            <textarea wire:model="comment"
                class="border-1 border-gray-100 shadow bg-opacity-20 rounded-lg placeholder-gray-500 w-full lg:w-1/2 h-32 focus:outline-none focus:ring-1 focus:border-green-500 focus:ring-green-500 dark:bg-gray-900 dark:border-transparent dark:text-gray-200"></textarea>
            <div>
                <button type="submit"
        ...
        @foreach ($recipe->comments as $comment)
            <div class="flex flex-row bg-gray-50 dark:bg-gray-600 shadow rounded p-4 mb-4">
                <div class="flex items-center justify-center flex-shrink-0 h-12 w-12 rounded-xl">
                    <img class="rounded-full w-10 h-10" src="{{ asset('user_img/' . $comment->user->avatar) }}"
                ...
                <div class="flex flex-col flex-grow ml-4">
                    <div class="text-xs font-light text-gray-500 dark:text-gray-300 pb-1">
                        ...
                        @if ($comment->user->id == Auth::id())
                            - <span class="italic cursor-pointer"
                                wire:click="deleteComment({{ $comment->id }})">{{ __('main.delete') }}</span>
                        @endif
                    </div>
