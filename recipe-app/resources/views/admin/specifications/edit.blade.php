...
@section('title', 'Edit - ' . $specification->name)
...
@section('admin.content')
    <div class="w-full p-4 bg-white border-b border-gray-200 rounded-t-md dark:bg-gray-700">
        <p class="font-bold dark:text-gray-200">
            {{ __('main.edit_specification') }} - {{ $specification->name }}
        </p>
    </div>
    <form action="{{ route('admin.specifications.update', $specification) }}" method="POST" class="p-4 space-y-4">
        @csrf
        @method('PATCH')
        <div class="grid grid-cols-1 lg:grid-cols-6">
            ...
            <input type="text" name="name" id="name" value="{{ old('name', $specification->name) }}"
